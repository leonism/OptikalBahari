...
require 'zlib'
require 'brotli'
begin; require 'zstd-ruby'; rescue LoadError; end
require 'thread'
...
module AssetProcessor
  ...
  class Configuration
    ...
    DEFAULT_CONFIG = {
      ...
      'compression' => {
        ...
        'zstd' => {
          'enabled' => true,
          'level' => 19  # 1-22, higher = better compression but slower
        },
  ...
  class TurboProcessor
    ...
    def compress_file_fast(file_path)
      ...
      if @config.enabled?('compression.zstd') && defined?(Zstd)
        futures << Concurrent::Future.execute(executor: thread_pool) do
          begin
    ...
    def add_existing_to_manifest(asset_path)
      ...
      hashed_file = cached_files.find { |f| f.include?('-') && !f.end_with?('.br', '.gz', '.zst') }
      if hashed_file

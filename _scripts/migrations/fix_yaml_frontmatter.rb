...
def fix_yaml_frontmatter(file_path)
  ...
  while i < frontmatter_lines.length
    ...
    if line.match(/^(\w+):\s*(.*)$/)
      ...
      if i + 1 < frontmatter_lines.length
        next_line = frontmatter_lines[i + 1]

        # If next line doesn't start with a key (no colon) and isn't empty, it's a continuation
        # Remove the indentation check that was causing issues
